...
#test on our own model

trainall = cbind(traindf, ytrain)
testdf$ytrain = NA

combinedpred = rbind(trainall, testdf)
...
prediction=knn(combinedpred, 384, 1:383, k=3)

prediction$ytrain

results = cbind(test2$Id, prediction$ytrain)
results = as.data.frame(results)
...
set.seed(10)
clusters <- kmeans(traindf, 4)


#to test plot elbow curve for clusters, 4 is best number of clusters
k.max <- 15
data <- combined
wss <- sapply(1:k.max, 
